...
import { FaQuestionCircle, FaCheckCircle, FaRegCalendarAlt } from 'react-icons/fa';
        ...
        >
            ...
            <motion.div className="flex items-center justify-between mb-4" variants={childVariants}>
                ...
                <div className="flex items-center space-x-2 text-gray-600">
                    <FaQuestionCircle />
                    <span className="text-lg">Question #{item.id}</span>
                </div>
            </motion.div>

            {/* Question */}
            <motion.div className="text-center mb-6" variants={childVariants}>
                <p className="text-xl font-semibold text-gray-800">{item.question}</p>
            </motion.div>

            {/* Options */}
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6" variants={childVariants}>
                {Object.keys(item.option).map((key) => (
                    ...
                    >
                        <div className="flex items-center space-x-2">
                            <span className="text-xl font-semibold text-gray-700">{key.toUpperCase()}.</span>
                            <span className="text-gray-600">{item.option[key]}</span>
                        </div>
                    </div>
                ))}
            </motion.div>
            ...
            <motion.div className="mb-6" variants={childVariants}>
                <div className="flex items-center space-x-2">
                    ...
                    <span className="text-lg font-bold text-green-600">{item.answer.toUpperCase()}</span>
                </div>
            </motion.div>

            {/* Metadata */}
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-500" variants={childVariants}>
                <div>
            ...
            {item.image && (
                <motion.div className="mt-4" variants={childVariants}>
                    <img src={item.image} alt="Question related" className="w-full h-auto rounded-md shadow-md" />
                </motion.div>
            )}
        </motion.div>
    );
